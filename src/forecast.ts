...
/**
 * Price a token INCREMENT at the tier its CUMULATIVE session usage settles in.
 *
 * THE ONE PLACE WHERE `priceUsage` IS DELIBERATELY NOT USED, and the reason is
 * the whole point of this function.
 *
 * `priceUsage` makes ONE tier selection from the session total, which is
 * correct for a session total: the total is what a single sum represents. A
 * DEFERRED DELTA is not a session total. It is the part of one that arrived
 * after the timeout snapshot, and the tokens in it were generated by model
 * calls whose prompt was the FULL context at that moment — not the increment.
 * So `promptSizeOf(increment)` is a meaningless number: it describes a context
 * size that never existed, and selecting a tier from it prices the increment at
 * a rate for a hypothetical prompt.
 *
 * The tier is therefore chosen from `settledUsage` — the session's cumulative
 * usage as read at settlement — and applied to `increment`. `settledUsage` is a
 * SEPARATE, EXPLICIT argument rather than something re-derived from the
 * increment, because re-deriving it is precisely the mistake being avoided and
 * would be invisible in review: both arguments have the same type, so a caller
 * that passed the increment twice would typecheck and be wrong by an order of
