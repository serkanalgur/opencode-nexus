...
- Keep docs close to code`
    ...
    await ctx.tool.transform((editor) => {
      ...
      editor.add({
        name: "goal.set",
        description: "Set a new persistent objective. Auto-completes any previous active goal.",
        input: {
          type: "object",
          properties: {
            description: { type: "string", description: "Goal description" },
            autoContinue: { type: "boolean", description: "Auto-continue until complete (default: true)" }
          },
        ...
        execute: async (input: unknown) => {
          ...
          return {
            content: [
              ...
              `📋 ID: ${goal.id}`,
              `🔄 Auto-continue: ${goal.autoContinue ? 'enabled' : 'disabled'}`,
              `📊 Status: ${goal.status}`
            ].join('\n')
          }
        }
